<script lang="ts">
	...
	import DatePickerCalendar from '$lib/gems/date-picker/DatePickerCalendar.svelte';
	...
	const containerClass = $derived(twMerge(
		'relative w-full rounded-md bg-control border border-base-b transition-colors cursor-pointer',
...
<div class={containerClass} role="none" onclick={openDropdown}>
	...
	<div class="flex h-full flex-row items-center pointer-events-none">
		...
		<span class={twMerge('flex-1 truncate text-control-c select-none', size === 'normal' ? 'pr-3' : 'pr-2')}>
			{#if value}
				{format(value)}
			{:else}
	...
	{#if clearable && value && !disabled}
		<button
			...
			onclick={clearValue}
			class={twMerge(
				'absolute right-0 top-0 h-full flex items-center justify-center shrink-0 rounded text-muted-c hover:text-control-c transition-colors z-20',
				iconPad,
			)}
			tabindex="-1"
		>
