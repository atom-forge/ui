<script lang="ts">
	...
	const wrapperClass = $derived(twMerge(
		'relative flex flex-col rounded-md border bg-control border-base-b transition-colors overflow-hidden',
		size === 'compact' && 'text-xs',
		size === 'small'   && 'text-[12px]',
		disabled && 'cursor-not-allowed bg-muted opacity-70 striped-dark-10',
		invalid  ? 'border-destructive text-destructive' : 'border-base-b',
		classes,
	));

	const taClass = $derived(twMerge(
		'w-full bg-transparent border-none focus:ring-0 disabled:cursor-not-allowed text-control-c placeholder:text-muted-c leading-5',
