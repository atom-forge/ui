<script lang="ts">
	...
	const wrapperClass = $derived(twMerge(
		'inline-flex items-center gap-2 select-none rounded-md py-1 px-2 transition-colors duration-150',
	...
	const boxClass = $derived(twMerge(
		...
		!!compact && 'w-4 h-4',
		!!small   && 'w-3.5 h-3.5',
		isActive && !primary && !accent && 'bg-control-v border-base-b text-control-c',
		isActive && !!primary           && 'bg-primary border-primary text-primary-c',
		isActive && !!accent            && 'bg-accent border-accent text-accent-c',
		!isActive && 'bg-control border-base-b',
	));
