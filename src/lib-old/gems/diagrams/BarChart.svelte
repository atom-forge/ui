<script lang="ts">
	import TooltipContent from "$lib/gems/diagrams/TooltipContent.svelte";
	import type {AnyProp, ClassProp} from "$lib/tools/types";
	...
	<div class="w-full h-full flex justify-around items-end gap-4 pt-4">
		{#each normalizedData as item, i}
			...
			>
					...
					{#if variant === 'grouped'}
						<div class="flex w-full h-full items-end gap-0.5">
							{#each item.values as segment}
								<div
									class="grow rounded-md transition-colors"
						...
						<div class="flex flex-col-reverse w-full h-full">
							{#each item.values as segment, j}
								<div
									...
									class:rounded-t-md={j === item.values.length - 1}
									class:rounded-b-md={j === 0}
									style:height="{(segment.value / maxValue) * 100}%"
									style:background-color={segment.color}
								></div>
							{/each}
				...
				<span class="text-xs text-muted-c shrink-0 whitespace-nowrap overflow-hidden text-ellipsis w-full text-center">{item.label}</span>
			</div>
		{/each}
	</div>
</div>
