<script lang="ts">
	...
	import TimePickerPopup from '$lib/gems/time-picker/TimePickerPopup.svelte';
	import type { XOR } from '$lib/tools/types';
	...
	const containerClass = $derived(twMerge(
		'relative w-full rounded-md bg-control border border-base-b transition-colors cursor-pointer select-none',
...
<div class={containerClass} role="none" onclick={openPopup}>
	...
	<div class="flex h-full flex-row items-center pointer-events-none">
		<span class={twMerge('flex items-center text-muted-c shrink-0', iconPad)}>
			<Icon icon={Clock} size="4" />
		</span>
		<span class={twMerge('flex-1 truncate text-control-c font-mono tabular-nums', size === 'normal' ? 'pr-3' : 'pr-2')}>
			{#if value}
				{formatDisplay(value)}
			{:else}
				<span class="text-muted-c font-sans">{placeholder}</span>
			{/if}
		</span>
	</div>
	...
	{#if clearable && value != null && !disabled}
		<button
			type="button"
			class={twMerge(
				'absolute right-0 top-0 h-full flex items-center justify-center shrink-0 text-muted-c hover:text-control-c transition-colors z-20',
				iconPad,
			)}
			onclick={clearValue}
			tabindex="-1"
