<script lang="ts">
	import type {ClassProp, XOR} from "$lib/tools/types";
...
<nav class={twMerge('flex items-center gap-1', classes)} aria-label="Pagination">
	<button
		...
		disabled={page <= 1}
		class={twMerge(
			'flex items-center justify-center rounded border border-base-b bg-control text-control-c',
			'hover:bg-control-m transition-colors disabled:opacity-40 disabled:cursor-not-allowed',
			btnSize
		)}
	...
	{#each items as item}
		{#if item === 'left' || item === 'right'}
			<span class={twMerge('flex items-center justify-center text-muted-c select-none', btnSize)}>
				…
			</span>
		{:else}
			<button
				...
				class={twMerge(
					...
					btnSize,
					item === page
						? 'bg-accent text-accent-c border-accent cursor-default'
						: 'bg-control text-control-c border-base-b hover:bg-control-m'
				)}
			>
				{item}
			</button>
