<script lang="ts">
	...
	import type {SliderSize} from '$lib/gems/slider/theme';
	import {getPercentage, getValueFromClientX} from '$lib/gems/slider/utils';
	...
	const rangeClass = untrack(() => twMerge(
		'absolute bg-accent striped-dark-10 shadow-inner z-10 top-1/2 -translate-y-1/2 rounded-full',
		size === 'normal'  && 'h-4',
		size === 'compact' && 'h-3',
		size === 'small'   && 'h-2',
	));
	const thumbClass = $derived(twMerge(
		'absolute bg-white rounded-full shadow-md border border-base-b cursor-pointer transform -translate-x-1/2 top-1/2 -translate-y-1/2 z-20',
		size === 'normal'  && 'h-5 w-5',
		size === 'compact' && 'h-4 w-4',
		size === 'small'   && 'h-3 w-3',
		disabled && 'cursor-not-allowed',
	));
	const stopClass = 'absolute w-1.5 h-1.5 bg-muted-c/50 rounded-full top-1/2 -translate-x-1/2 -translate-y-1/2';
			...
			{#each percents as percent, i}
				...
				>
					{#if showValue && isDragging && activeThumbIndex === i}
						<div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-1.5 py-0.5 rounded bg-control border border-base-b shadow-sm text-xs text-control-c tabular-nums whitespace-nowrap pointer-events-none select-none">
