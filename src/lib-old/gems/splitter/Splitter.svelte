<script lang="ts">
	...
	let startSize = $state(untrack(() => Math.max(minSize, Math.min(100 - minSize, initialSize))));
...
<div
	bind:this={container}
	class={twMerge('flex overflow-hidden h-full', isVertical ? 'flex-row' : 'flex-col', rootClass)}
>
	<div
		id={startId}
	...
	<div
		...
		class={twMerge(
			'flex-none flex items-center justify-center select-none focus:outline-none focus-visible:ring-2 focus-visible:ring-accent transition-colors',
			isVertical
	...
	>
		...
		{:else}
			<div class={twMerge(
				'rounded-full bg-border opacity-60',
				isVertical ? 'w-px h-8' : 'h-px w-8',
			)}></div>
		{/if}
	</div>
