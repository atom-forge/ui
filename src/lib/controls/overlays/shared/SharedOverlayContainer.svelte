...
{#each manager.items as item, i (item.id)}
	{@const Component = item.component}
	<div
		role="none"
		class={`fixed inset-0 bg-transparent ${item.kind === 'modal' ? 'flex flex-row items-center justify-center' : ''}`}
		style:z-index={overlayZIndex(i)}
