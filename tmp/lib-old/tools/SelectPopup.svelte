<script lang="ts">
	...
	import {Kbd} from '$lib/gems/kbd';
	import Spinner from '../../../src/lib/tools/Spinner.svelte';
	import EmptyState from '../../../src/lib/gems/empty-state/EmptyState.svelte';
	import {Card, type ClassProp} from '$lib';
...
<Card class={twMerge('max-h-[40vh] flex flex-col', classes)}>
	...
	>
		{#if searchable}
			<div class="flex items-center gap-3 px-4 border-b border-base-b shrink-0">
				{#if loading}
					<Spinner class="w-4 h-4 text-accent shrink-0"/>
				{:else}
					<span class="text-muted-c shrink-0">
				...
				<input
					...
					type="text"
					{placeholder}
					class="flex-1 min-w-0 bg-transparent outline-none focus:ring-0 border-0 py-3 text-sm text-control-c placeholder:text-muted-c"
				/>
			</div>
		{/if}
		...
		<div role="listbox" class="overflow-y-auto flex-1 min-h-0 py-2" bind:this={listEl}>
			...
			{:else if flatItems.length === 0}
				...
				{:else}
					<div class="px-4 py-8 text-center text-sm text-muted-c">No items available.</div>
				{/if}
			{:else if grouped}
				{#each groups as g, gi}
					{@const offset = groups.slice(0, gi).reduce((sum, pg) => sum + pg.items.length, 0)}
					<div class="px-3 pt-3 pb-1">
						<span class="text-[10px] font-semibold uppercase tracking-wider text-muted-c">{g.group}</span>
					</div>
		...
		<div class="flex items-center gap-1.5 px-4 py-2 border-t border-base-b shrink-0">
			<Kbd keys={['↑']}/>
			<Kbd keys={['↓']}/>
			<span class="mx-1 w-px h-3 bg-base-b"></span>
			<Kbd keys={['↩']}/>
