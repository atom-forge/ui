<script lang="ts">
	import {getAccordionManager} from '$lib/gems/accordion/accordion-manager.svelte.js';
	import {Icon, type IconDefinition} from "$lib/gems/icon";
	import type {AnyProp, ChildrenProp, ClassProp} from "$lib/tools/types";
	...
	const wrapperClass = $derived(twMerge(
		'overflow-hidden',
		!manager.borderless && !manager.joined && 'border border-base-b rounded-lg',
		classes
	));

	const buttonClass = $derived(twMerge(
		'w-full flex justify-between items-center text-left font-medium text-control-c bg-control-v hover:bg-control-m transition-colors',
	...
	const contentClass = $derived(twMerge(
		(manager.joined || !manager.borderless) && 'border-t border-base-b',
	));
</script>
