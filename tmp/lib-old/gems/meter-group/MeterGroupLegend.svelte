<script lang="ts">
	...
	const listClass = twMerge(
		vertical ? 'flex flex-col' : 'flex flex-wrap',
		size === 'normal' && (vertical ? 'gap-2' : 'gap-x-4 gap-y-2'),
		size === 'compact' && (vertical ? 'gap-1.5' : 'gap-x-3 gap-y-1.5'),
		size === 'small' && (vertical ? 'gap-1' : 'gap-x-2 gap-y-1'),
	);
	...
	{#each items as meterItem}
		...
		>
			...
			{:else}
				...
				></span>
				<span class={twMerge('text-control-c', labelSize, labelClass)}>{meterItem.label}</span>
				{#if showValues}
					{#if valueSnippet}
					...
					{:else}
						<span
							class={twMerge(
								'text-muted-c font-medium tabular-nums',
								vertical && 'ml-auto',
								valueSize,
								valueClass,
