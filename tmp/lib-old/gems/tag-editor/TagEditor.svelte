<script lang="ts">
	...
	import type {FlatItem} from '$lib/tools/SelectPopup.svelte';
	import type {XOR, ClassProp} from '$lib';
	...
	const containerCls = $derived(twMerge(
		'flex flex-wrap items-center gap-1 w-full px-3 rounded-lg border bg-control transition-colors cursor-text',
		size === 'normal'  && 'min-h-10 py-1.5 text-sm',
		size === 'compact' && 'min-h-8 py-1 text-xs',
		size === 'small'   && 'min-h-6 py-0.5 text-xs px-2',
		isOpen  ? 'border-accent ring-2 ring-accent/20' : 'border-base-b hover:border-accent/60',
...
<div role="presentation" bind:this={containerEl} class={containerCls} onclick={() => inputEl?.focus()}>
	{#if sortable}
		...
		>
			{#snippet item(si)}
				...
				{:else}
					<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-accent text-accent-c text-xs font-medium cursor-grab active:cursor-grabbing select-none shrink-0">
						{si.tag}
	...
	{:else}
		{#each value as tag}
			...
			{:else}
				<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-accent text-accent-c text-xs font-medium shrink-0">
					{tag}
					<!-- svelte-ignore a11y_click_events_have_key_events a11y_interactive_supports_focus -->
					<span
						role="button"
	...
	<input
		...
		class="flex-1 min-w-20 bg-transparent outline-none py-0 px-2 focus:ring-0 border-0 placeholder:text-muted-c text-control-c"
	/>
	...
	{#if clearable && value.length > 0}
		...
		<span
			...
			class="text-muted-c hover:text-control-c shrink-0 ml-1"
		>
