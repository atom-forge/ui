<script lang="ts">
	...
	const itemClass = (idx: number) => twMerge(
		'px-3 py-1.5 cursor-pointer text-sm text-control-c truncate',
		idx === data.activeIndex ? 'bg-accent text-accent-c' : 'hover:bg-control-m',
	);
</script>
...
<Card class="p-0 overflow-hidden">
	<ul bind:this={listEl} class="max-h-48 overflow-y-auto py-1" role="listbox">
		{#if data.loading}
			<li class="px-3 py-2 text-sm text-muted-c">Loading…</li>
		{:else}
