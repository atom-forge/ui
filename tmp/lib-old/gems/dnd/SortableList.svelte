<script lang="ts" generics="T extends { id: string | number }">
	...
	import {dndDraggable, dndDropTarget, getAdapter, getSetCustomNativeDragPreview, getPointerOutsideOfPreview} from "./actions";
	...
	let setCustomNativeDragPreview: Awaited<ReturnType<typeof getSetCustomNativeDragPreview>> = () => {};
	let pointerOutsideOfPreview: Awaited<ReturnType<typeof getPointerOutsideOfPreview>> = () => ({x: 0, y: 0}) as any;
	...
	onMount(async () => {
		const [hitbox, { monitorForElements }, preview, pointer] = await Promise.all([
			...
			getAdapter(),
			getSetCustomNativeDragPreview(),
			getPointerOutsideOfPreview(),
		]);
...
{#snippet slot(atIndex: number)}
	{#if insertIndex === atIndex}
