...
>
	...
	{#if hoveredCell}
		...
		>
			...
			{:else}
				<div class="bg-raised border border-base-b rounded-lg shadow-lg px-3 py-2 text-sm min-w-[120px]">
					<div class="font-medium text-control-c">{hoveredCell.yLabel}, {hoveredCell.xLabel}</div>
					<div class="text-muted-c text-xs mt-0.5">{hoveredCell.value}</div>
				</div>
			{/if}
		</div>
	{/if}
