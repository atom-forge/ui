<script lang="ts">
	...
	import type {ClassProp, ChildrenProp} from '$lib/tools/types';
...
{#if isHorizontal}
	...
	<div class={twMerge('flex items-center gap-1 pt-2 min-w-0', classes)}>
		{#if labelSnippet}
			{@render labelSnippet()}
		{:else if label}
			<label for={forId} class="text-sm text-muted-c leading-snug">
				{label}
				{#if required}<span class="text-destructive ml-0.5">*</span>{/if}
				{#if optional}<span class="text-muted-c text-xs ml-1">(optional)</span>{/if}
			</label>
		{/if}
		{#if action}
		...
		{#if label || labelSnippet || action}
			<div class="flex items-center gap-1">
				{#if labelSnippet}
					{@render labelSnippet()}
				{:else if label}
					<label for={forId} class="text-sm font-medium text-control-c leading-snug">
						{label}
						{#if required}<span class="text-destructive ml-0.5">*</span>{/if}
						{#if optional}<span class="text-muted-c text-xs ml-1">(optional)</span>{/if}
					</label>
