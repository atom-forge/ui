<script lang="ts">
	...
	import Avatar from "../../../../src/lib/gems/avatar/Avatar.svelte";
...
<div class={twMerge('flex items-center', classes)}>
	...
	{#if overflow > 0}
		<div class={twMerge(
			'-ml-1.5 rounded-full ring-2 ring-[var(--color-canvas)] flex items-center justify-center bg-control-m text-muted-c font-bold shrink-0 select-none',
			sizeClass
		)}>+{overflow}</div>
	{/if}
</div>
