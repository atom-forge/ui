<script lang="ts">
	import type {ChildrenProp, ClassProp} from "$lib";
...
{#if show && (label || content)}
	<div
		use:portal={"#atom-forge-ui:portal-target"}
		class={twMerge(
			"fixed z-10000 px-2 py-1 text-xs rounded-md shadow-lg",
			inverted
				? "bg-control-c text-base border-transparent"
				: "bg-control border border-base-b text-control-c"
		)}
		style={style}
