<script lang="ts">
	...
	import type {SelectOption, SelectOptionsSource} from '../../../../src/lib/gems/select/Select.svelte';
	...
	const triggerCls = $derived(twMerge(
		'w-full flex items-center gap-2 px-3 rounded-lg border bg-control text-left transition-colors',
		size === 'normal'  && 'min-h-10 py-1.5 text-sm',
		size === 'compact' && 'min-h-8 py-1 text-xs',
		size === 'small'   && 'min-h-6 py-0.5 text-xs px-2',
		isOpen ? 'border-accent ring-2 ring-accent/20' : 'border-base-b',
...
>
	<span class="flex-1 flex flex-wrap gap-1 min-w-0" role="presentation">
		{#if selectedOptions.length === 0}
			<span class="text-muted-c">{placeholder}</span>
		{:else if isSortable}
			<SortableList
			...
			>
				{#snippet item(chipItem)}
					...
					{:else}
						<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-accent text-accent-c text-xs font-medium cursor-grab active:cursor-grabbing select-none">
							{chipItem.label}
		...
		{:else}
			{#each selectedOptions as opt}
				...
				{:else}
					<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-accent text-accent-c text-xs font-medium">
						{opt.label}
						<!-- svelte-ignore a11y_click_events_have_key_events a11y_interactive_supports_focus -->
						<span
							role="button"
	...
	{#if clearable && value.length > 0}
		...
		<span
			...
			class="text-muted-c hover:text-control-c shrink-0"
		>
	...
	<Icon
		icon={ChevronDown}
		pxSize={14}
		class={twMerge('text-muted-c shrink-0 transition-transform duration-150', isOpen && 'rotate-180')}
	/>
</button>
