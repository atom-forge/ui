<script lang="ts">
	import {Button, Card, defineIcon, getPopupManager, PopupContainer} from "$lib"
	import {twMerge} from "tailwind-merge";
	import {ChevronRight} from 'lucide-svelte';
	import ContextMenu from '../../../../src/lib/gems/context-menu/ContextMenu.svelte';
	import {type ContextMenuItemConfig, type ContextMenuItemConfigAny} from "$lib/gems/context-menu/types";
...
<PopupContainer>
	...
	<Card class="shadow-2xl p-1 flex flex-col gap-0">
		{#each config as item}
			{#if item.separator}
				<hr class="my-1 bg-base-b h-px border-0"/>
			{:else}
				{@const showChevron = item.chevron ?? ("submenu" in item && !!item.submenu)}
				<Button
					ghost compact
					class={twMerge('w-full justify-between', item.warning && 'text-destructive')}
					icon={item.icon}
					label={item.label}
