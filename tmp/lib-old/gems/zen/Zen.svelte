...
{#if active}
	<div
		...
		aria-modal="true"
		tabindex="-1"
		class={twMerge('fixed inset-0 z-9998 bg-black/60 backdrop-blur-sm', backdropClass)}
		...
		>
			...
			{#if title}
				<div class="flex items-center justify-center backdrop-blur-sm shrink-0 py-3">
					<span class="text-sm font-medium use-control px-3 py-1 rounded-2xl border border-base-b">{title}</span>
				</div>
			{/if}
		</div>
	</div>
