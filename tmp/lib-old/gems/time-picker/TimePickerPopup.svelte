<script lang="ts">
	...
	import { Button } from '$lib/gems/button';
	...
	const inputClass = 'w-12 h-11 text-center text-lg font-semibold font-mono tabular-nums bg-secondary rounded-md focus:outline-none focus:ring-2 focus:ring-accent text-control-c cursor-default select-none';
	const btnClass   = 'flex items-center justify-center w-8 h-7 rounded text-muted-c hover:text-control-c hover:bg-secondary transition-colors';
</script>
	...
	<div class="flex items-start justify-center gap-1">
		...
		{#snippet spinCol(label: string, val: number, onUp: () => void, onDown: () => void, setVal: (n: number) => void)}
			<div class="flex flex-col items-center gap-1">
				...
				<span class="text-[11px] text-muted-c uppercase tracking-wide">{label}</span>
			</div>
		{/snippet}

		{#snippet sep()}
			<span class="text-2xl font-bold text-muted-c mt-3 select-none">:</span>
		{/snippet}
