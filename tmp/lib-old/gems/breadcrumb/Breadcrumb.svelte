<script lang="ts">
	import type {ClassProp} from "$lib/tools/types";
	import {twMerge} from "tailwind-merge";
	import {getPopupManager} from "$lib/gems/popup/popup-manager.svelte.js";
	import {ContextMenu, type ContextMenuItemConfig} from "$lib/gems/context-menu";
...
<nav aria-label="Breadcrumb" class={twMerge('flex items-center gap-1 text-sm', classes)}>
	{#each visibleItems() as item, i}
		...
		{#if i > 0}
			<span class="text-muted-c select-none">/</span>
		{/if}
		{#if item.collapsed}
			<button
				type="button"
				onclick={(e) => openCollapsed(e, item.hidden)}
				class="px-1.5 py-0.5 text-xs text-muted-c hover:text-control-c hover:bg-base-b rounded transition-colors cursor-pointer leading-none"
		...
		{:else if (item.href || item.onclick) && !isLast}
			<a
				...
				class="text-muted-c hover:text-control-c transition-colors truncate max-w-32"
				title={item.label}
			>{truncate(item.label)}</a>
		{:else}
			<span class="text-control-c font-medium truncate max-w-48" title={item.label}>{truncate(item.label)}</span>
		{/if}
	{/each}
</nav>
