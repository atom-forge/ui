...
<label class={wrapperClass}>
	...
	{#if label}
		{#if typeof label === 'string'}
			<span class="ms-3 text-sm font-medium text-control-c">{label}</span>
		{:else}
			<div class="ms-3 grid grid-cols-1 grid-rows-1 text-sm font-medium text-control-c">
