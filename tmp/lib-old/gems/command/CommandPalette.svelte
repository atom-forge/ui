<script lang="ts">
	...
	import SelectPopup, {type GroupedItems} from '$lib/tools/SelectPopup.svelte';
	import {Icon} from '$lib/gems/icon';
	import type {CommandItem} from './command.svelte.js';
	...
	{#snippet itemRenderer(data, isHighlighted)}
		...
		{:else}
			<div class={twMerge(
				...
				isHighlighted ? 'bg-accent text-accent-c' : 'hover:bg-base-b',
			)}>
				{#if data.icon}
