<script lang="ts">
	...
	const sepClass = untrack(() => twMerge(
		'text-muted-c font-mono select-none px-0.5',
		size === 'normal'  && 'text-sm',
	...
	const boxClass = (idx: number) => twMerge(
		...
		'bg-control border-base-b text-control-c transition-colors select-none',
		boxSize,
	...
	const placeholderClass = 'text-muted-c/40 pointer-events-none';
</script>
